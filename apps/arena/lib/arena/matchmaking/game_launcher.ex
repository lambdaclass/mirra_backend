defmodule Arena.Matchmaking.GameLauncher do
  ...
  def handle_info(:launch_game?, %{clients: clients} = state) do
    ...
    launch_game_interval = if is_loadtest_alone_mode?, do: 30, else: 300
    ...
    amount_of_players = if is_loadtest_alone_mode?, do: 1, else: state.current_map.amount_of_players
    ...
    if Map.has_key?(state, :game_mode_configuration) &&
         (has_enough_players? or
            is_queue_timed_out?) do
  ...
  def handle_info(:start_game, state) do
    amount_of_players = if System.get_env("LOADTEST_ALONE_MODE") == "true", do: 1, else: state.current_map.amount_of_players
